...
function startGame() {
  ...
  shuffledQuestions = questions.sort(() => Math.random() - 0.5); // Xáo trộn câu hỏi
...
function setNextQuestion() {
  ...
  showQuestion(shuffledQuestions[currentQuestionIndex]); // Hiển thị câu hỏi hiện tại
}
...
function showQuestion(question) {
  ...
  question.answers.forEach((answer) => {
    ...
    button.innerText = answer.text; // Hiển thị văn bản của câu trả lời
    button.classList.add("btn");
    if (answer.correct) {
      button.dataset.correct = answer.correct; // Đánh dấu câu trả lời đúng
    }
    button.addEventListener("click", selectAnswer); // Gắn sự kiện chọn câu trả lời
...
function selectAnswer(e) {
  ...
  setStatusClass(document.body, correct); // Đặt trạng thái đúng/sai cho body
  Array.from(answerButtonsElement.children).forEach((button) => {
    setStatusClass(button, button.dataset.correct); // Đặt trạng thái đúng/sai cho từng nút
  });
  // Nếu còn câu hỏi, hiển thị nút Next. Ngược lại, hiển thị nút Restart.
  if (shuffledQuestions.length > currentQuestionIndex + 1) {
    nextButton.classList.remove("hide");
  } else {
    startButton.innerText = "Restart";
    startButton.classList.remove("hide");
    translateButton.classList.add("hide"); // Ẩn nút dịch khi kết thúc trò chơi
  }
}
...
function toggleTranslation() {
  ...
  if (translatedQuestionElement.classList.contains("hide")) {
    ...
    translatedQuestionElement.innerText = `(Bản dịch): ${currentQuestion.translation.vi}`;
    translatedQuestionElement.classList.remove("hide");

    // HIỂN THỊ BẢN DỊCH CÁC CÂU TRẢ LỜI
    const translatedAnswers = currentQuestion.answers.map(answer => {
      // Đảm bảo mỗi câu trả lời có bản dịch
      return answer.translation && answer.translation.vi ? answer.translation.vi : answer.text;
    });
    // *** THAY ĐỔI DÒNG NÀY ***
    translatedAnswersElement.innerHTML = `(Đáp án dịch):<br>${translatedAnswers.join('<br>')}`; // Nối các bản dịch bằng thẻ <br>
